    ...
    <div className="relative" ref={wrapperRef}>
        ...
        <div className="flex items-center justify-between p-3 border border-indigo-200 bg-indigo-50 rounded-lg">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-bold">
              {(
                selectedUser.first_name?.[0] || selectedUser.display_contact[0]
              ).toUpperCase()}
            </div>
            <div>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            name="first_name"
            type="text"
            required={hasSelectedUser}
            disabled={!hasSelectedUser}
            value={firstName}
            onChange={(e) => setFirstName(e.target.value)}
            placeholder="Enter first name"
            className="w-full rounded-lg border border-gray-300 px-4 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 disabled:bg-gray-50 disabled:text-gray-400"
          />
        </div>
      ...
      <div className="mt-4">
        ...
        <p className="mt-2 text-xs text-gray-500">
          {selectedUser
            ? selectedUser.email
              ? "Update this only if the admin should use a different public email."
              : "Phone-only users need a real email here before they can be added to the team."
            : "Select a user to prefill and confirm their admin identity."}
        </p>
      </div>
            ...
            users.map((user) => (
              ...
              >
                <div className="h-8 w-8 flex-shrink-0 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 text-xs font-bold">
                  {(
                    user.first_name?.[0] || user.display_contact[0]
                  ).toUpperCase()}
                </div>
                <div>
