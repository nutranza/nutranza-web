...
export const getCustomerOrderPageContent = (
  ...
  if (SUCCESS_PAYMENT_STATUSES.has(paymentStatus)) {
    return {
      ...
      description:
        "Thank you for shopping with Nutranza Foods. We're getting your order ready for delivery.",
      showTracking: true,
    }
  }
...
export const getCustomerOrderPageMetadata = (
  ...
  if (content.state === "payment_pending") {
    return {
      title: `Payment Pending #${order.display_id} | Nutranza Foods`,
  ...
  if (content.state === "cancelled") {
    return {
      title: `${content.title} #${order.display_id} | Nutranza Foods`,
      description: content.description,
    }
  }

  return {
    title: `Order Confirmed #${order.display_id} | Nutranza Foods`,
