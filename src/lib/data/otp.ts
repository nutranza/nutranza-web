...
const DEFAULT_WHATSAPP_LOGIN_EMAIL_DOMAIN = "wa.nutranza.local"
...
async function handoffGuestCartForUser(
  ...
  const cartId = cookieStore.get("nutranza_cart_id")?.value?.trim()
  ...
  if (activeCartId) {
    cookieStore.set("nutranza_cart_id", activeCartId, {
      maxAge: 60 * 60 * 24 * 7,
      httpOnly: true,
      sameSite: "strict",
