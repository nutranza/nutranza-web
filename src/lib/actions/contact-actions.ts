...
export async function sendContactEmail(
    ...
    try {
        ...
        const recipientEmail = process.env.CONTACT_FORM_RECIPIENT || "support@nutranza.local"
        const senderEmail = process.env.CONTACT_FORM_SENDER || "website@nutranza.local"
      ...
      <div style="font-family: sans-serif; line-height: 1.6; color: #333;">
        ...
        <hr style="border: 0; border-top: 1px solid #eee;" />
        <p style="font-size: 0.8em; color: #888;">This email was sent from the Nutranza Foods contact form.</p>
      </div>
    `

        await sendBrevoEmail({
            subject: `New Inquiry from ${validatedData.name} - Nutranza Foods`,
            htmlContent,
            sender: {
                email: senderEmail,
                name: "Nutranza Foods Website",
            },
            to: [
                {
                    email: recipientEmail,
                    name: "Nutranza Customer Care",
                },
            ],
            replyTo: {
...
export async function sendProductQuestion(
    ...
    try {
      ...
      <div style="font-family: sans-serif; line-height: 1.6; color: #333;">
        ...
        <p style="white-space: pre-wrap; background: #f9f9f9; padding: 15px; border-radius: 5px;">${validatedData.message}</p>
        <hr style="border: 0; border-top: 1px solid #eee;" />
        <p style="font-size: 0.8em; color: #888;">This email was sent from the "Ask a Question" form on a Nutranza product page.</p>
      </div>
    `
