...
export function mapCommerceProduct(product: CommerceProduct): WebsiteProduct {
  ...
  const primaryImage = product.image_url || product.thumbnail || gallery[0] || DEFAULT_IMAGE
  const prices = (variants.length ? variants.map((variant) => variant.price) : [product.price])
  ...
  return {
    ...
    image: primaryImage,
    imageAlt: stringValue(metadata.image_alt, `${product.name || product.title} product image`),
    ...
    rating: Math.max(0, Math.min(5, Math.round(numberValue(metadata.rating, 0)))),
    reviewCount: Math.max(0, Math.round(numberValue(metadata.review_count, 0))),
    ...
    description: stringList(metadata.description_sections, description ? [description] : []),
    stats,
